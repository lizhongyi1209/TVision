...
import { setMainFromFiles } from "@/lib/canvasFiles";
...
export function Stage() {
  ...
  // The reset flow (1st file = full-res main image, rest = refs, TVision PNG
  // metadata restored, multi-canvas-tab-switch guarded) lives in
  // lib/canvasFiles.ts, shared with the dock's 图片素材 section.
  const addFiles = useCallback(async (files: File[]) => {
    if (!files.length) return;
    setBusy(true);
    try {
      await setMainFromFiles(files);
    } finally {
      setBusy(false);
    }
  }, []);
