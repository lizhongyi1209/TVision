...
export async function GET() {
  ...
  return NextResponse.json({ items: await readTemplates(auth.uid) });
}
...
export async function POST(req: Request) {
  ...
  const items: Template[] = await upsertTemplate(auth.uid, { id, name, notes, count: sanitizeCount(body.count), ...params });
...
export async function DELETE(req: Request) {
  ...
  return NextResponse.json({ items: await deleteTemplate(auth.uid, id) });
}
